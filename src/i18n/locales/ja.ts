const ja = {
  ...
  premium: {
    ...
    planOneTime: '一回払い',
    planBestValue: 'お得',
    planPriceUnavailable: '取得できませんでした',
    planRetry: '再読み込み',
  ...
  // ── バックグラウンド位置情報 認識しやすい開示 ───────────
  backgroundDisclosure: {
    title: 'バックグラウンド位置情報の利用について',
    body: 'Yorimichi は、登録した場所（ジオフェンス）に近づいたときにリマインド通知をお届けするために、バックグラウンドで位置情報を収集します。\n\nこの機能はアプリが閉じているとき、または使用していないときにも動作します。\n\n【収集するデータ】\n端末の位置情報（緯度・経度）\n\n【利用目的】\n登録した場所への接近・離脱を検知し、チェックリストのリマインド通知を送信するためだけに使用します。位置情報は広告やトラッキングには一切使用しません。\n\n次の画面で「常に許可」を選択してください。',
    accept: '同意して続ける',
    decline: '今はしない',
  },
  // ── App 起動時の権限ダイアログ ──────────────────────────
