...
| プレミアム | なし | 無制限 | 制限なし ※ | アラーム対応 | 複数人 |

> ※ Android `GeofencingClient` の API ハードリミットにより、**アプリ全体で最大100件**（全メモ合計）登録可能。1メモあたりの上限はアプリ側では設けていないが、全地点合計が100件を超えた分は登録されない。
...
### 共有メモの同期タイミング整理（2026-03-10）

> 「自動反映」という表現は実態と乖離するため廃止。同期が発生するタイミングを明確に定義。

| タイミング | プレミアム | 無料 | 対応ファイル | コミット |
|---|---|---|---|---|
| メモ一覧へ遷移時 | shareId を持つ全メモを並列 pull してローカルを更新 | 同左 | `MemoListScreen.tsx`, `shareService.ts` | — |
| メモ詳細を開いたとき（マウント時） | `onSnapshot` リスナーを開始（画面を閉じると停止） | 1回だけ pull | `MemoDetailScreen.tsx` | — |
| 手動同期ボタン | 1回 pull | 1回 pull | `MemoDetailScreen.tsx` | — |
| アイテムのチェック操作 | Firestore に即時 push（fire-and-forget） | 同左 | `MemoDetailScreen.tsx`, `shareService.ts` | — |
| 地点の追加・編集・削除（オーナーのみ） | Firestore に即時 push | 同左 | `MemoDetailScreen.tsx`, `shareService.ts` | — |
| バックグラウンド / アプリ終了時 | **同期なし** | **同期なし** | — | — |

**プレミアムとの差別化:** 詳細画面を開いている間のリアルタイム反映（`onSnapshot`）はプレミアム限定。一覧遷移時・詳細起動時の pull は両プラン共通。
...
| 1メモあたり登録地点数 | 2箇所まで | 制限なし ※アプリ全体で最大100件（Android API 制約）|
| 共有人数 | 1対1（2人）まで | 無制限 |
| カテゴリ選択（プリセット） | ✅ | ✅ |
| タグ付け | 3タグまで/メモ | 無制限 |
| カスタムカテゴリ作成 | ✗ | ✅ |
